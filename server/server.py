    ...
    async for msg in ws:
        ...
        if t == "CREATE_ROOM":
            ...
            for i,p in enumerate(room["players"]):
                ...
                room["scores"][p] = 0
                await room["sockets"][i].send(json.dumps({"type":"TICKET_ASSIGNED","data":{"ticket":room["tickets"][p]}}))
            ...
            if claim in room["claimed"]:
                await ws.send(json.dumps({"type":"CLAIM_RESULT","data":{"status":"ALREADY","claim":claim}}))
                continue
            ...
            valid = False
            if claim=="QUICK_5": valid=len(drawn)>=5
            ...
            elif claim=="FIRST_LINE": valid=all(n in drawn for n in rows[0])
            elif claim=="SECOND_LINE": valid=all(n in drawn for n in rows[1])
            elif claim=="THIRD_LINE": valid=all(n in drawn for n in rows[2])
            elif claim=="TAMBOLA": valid=all(n in drawn for n in all_nums)

            if not valid:
                await ws.send(json.dumps({"type":"CLAIM_RESULT","data":{"status":"INVALID","claim":claim}}))
                continue

            room["claimed"].add(claim)
            room["scores"][player]+=1
            await broadcast(room,{"type":"SCORE_UPDATE","data":{"scores":room["scores"]}})
            await broadcast(room,{"type":"CLAIM_RESULT","data":{"status":"SUCCESS","claim":claim,"player":player}})

            if claim=="TAMBOLA":
                room["ended"]=True
                leaderboard=sorted(room["scores"].items(),key=lambda x:x[1],reverse=True)
                await broadcast(room,{"type":"GAME_ENDED","data":{"leaderboard":[{"name":p,"score":s} for p,s in leaderboard]}})
